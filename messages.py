...
def get_messages(thread_id):
    sql = "SELECT messages.sent_by, messages.id, users.username, messages.content, messages.sent_at, messages.modified FROM users, messages WHERE messages.sent_by = users.id AND messages.thread_id =:thread AND messages.visible > 0 ORDER BY messages.sent_at, messages.id;"
